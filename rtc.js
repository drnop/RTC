...
Config.prototype.editConfig = function() {
	...
	var input_type = this.config[i].type;
	if (input_type == "textarea") {
	    var value = this.config[i].value;
	    addCellTextarea(divRow,input_type,this.config[i].id,this.config[i].id,value);
	}
	if (input_type == "select") {
	    if (this.config[i].multiple) {
	...
	if (input_type == "text" || input_type == "password") {
	    ...
	    addCellInput(divRow,input_type,this.config[i].id,this.config[i].id,value);
	}
...
Config.prototype.getConfigResponse = function(xhr,th) {
	...
	for (var i=0;i<th.config.length;i++) {
		...
		// don't set config to undefined, keep previus value, "" or []
	    }
	    else {
...
Config.prototype.updateConfig = function(th) {
    ...
    for (var i=0;i<th.config.length;i++) {
	id = th.config[i].id;
	t_type = th.config[i].type;

	if (t_type == "select") {
	    var item = document.getElementById(th.config[i].id);
	    var options = [];
...
function SWconfig(){
	...
	{ "type": "password", "id" :"sw_password","value":"", "label": "SW PASSWORD" },
	{  "type": "select", "multiple":10,"id" :"hostgroups","value":[], "label": "SELECT HOST GROUPS","type":"select","options":[] },	
    ];
    
    Config.call(this,config,"swconfig","Stealthwatch Configuration");
}
SWconfig.prototype.setHostgroups = function(hostgroups) {
    this.config[3].options = hostgroups;
}
...
function ISEconfig(){
	...
	{ "type": "password", "id" :"ise_password","value":"", "label": "ISE PASSWORD" },
	...
	{ "type": "textarea", "id" :"pxgrid_client_cert","value":"", "label": "PXGRID CLIENT CERT" },
	{ "type": "textarea", "id" :"pxgrid_client_key","value":"", "label": "PXGRID CLIENT KEY" },
	{ "type": "text", "id" :"pxgrid_client_key_pw","value":"", "label": "PXGRID CLIENT KEY PASSWORD" },
	{ "type": "textarea", "id" :"pxgrid_server_cert","value":"", "label": "PXGRID SERVER CERT" }
    ];
...
function ADconfig(){
	...
	{ "type": "password", "id" :"ad_password","value":"", "label": "AD PASSWORD" },
	{  "type": "select", "multiple":10,"id" :"ad_groups","value":[], "label": "SELECT AD GROUPS","type":"select","options":[] },	

    ];
    Config.call(this,config,"adconfig","Active Directory Configuration");
}
ADconfig.prototype.setADgroups = function(adgroups) {
    this.config[4].options = adgroups;
}

SMAconfig.prototype = Object.create(Config.prototype);
SMAconfig.prototype.constructor = SMAconfig;
function SMAconfig(){
    config = [

	{ "type": "text","id" :"sma_server","value":"", "label": "SMA SERVER" },
	{ "type": "text","id" :"sma_username","value":"", "label": "SMA USERNAME" },
	{ "type": "password","id" :"sma_password","value":"", "label": "SMA PASSWORD" }	
    ];
    Config.call(this,config,"smaconfig","SMA Configuration");
}

DUOconfig.prototype = Object.create(Config.prototype);
DUOconfig.prototype.constructor = DUOconfig;
function DUOconfig(){
    config = [

	{ "type": "text","id" :"duo_host","value":"", "label": "DUOHOST" },
	{ "type": "password","id" :"api_ikey","value":"", "label": "API IKEY" },
	{ "type": "password","id" :"api_skey","value":"", "label": "API SKEY" }	
    ];
    Config.call(this,config,"duoconfig","DUO Configuration");
}
...
StoredEvents.prototype.cbDLPreport = function(th,id) {

    alert("Getting flow currently only supported for Users");
    var name = this.id;    
    return;
}
...
StoredEvents.prototype.showItem = function(item) {
    ...
    addCellwCallback3(divRow,"fas fa-address-book",item[this.key],this.cbDLPreport,"DLP Report",this);			    
...
function MACsStoredEvents(){
    ...
    StoredEvents.call(this,"fas fa-desktop","mac","/cgi-bin/rtcGetMACs.py",[" Penalty","ANC","CTR","AMP","UMB","SW","Flows","DLP Report","Device","MAC","Purge"],"Hosts/MAC addresses");
}
...
function UsersStoredEvents(){
    StoredEvents.call(this,"fas fa-user","user","/cgi-bin/rtcGetUsers.py",[" Penalty","Quarantine"," CTR","AMP","UMB","SW","Flows","DLP Report","Details","Username","Purge"],"Users");
}
...
UsersStoredEvents.prototype.cbDLPreport = function(th,id) {
    var item = th.getItem(id);
    //    alert(JSON.stringify(item));
    mDLPreport(item["ad_info"]["sAMAccountName"],item["ad_info"]["mail"]);
}
...
UsersStoredEvents.prototype.showItemDetails = function(user) {
	...
	addRow(divDetails,"Account",user["ad_info"]["sAMAccountName"]);
...
function HostnamesStoredEvents(){
    StoredEvents.call(this,"fas fa-desktop","hostname","/cgi-bin/rtcGetHostnames.py",[" Penalty","Isolated"," CTR","AMP","UMB","SW","Flows","DLP Report","Details","Hostnames","Purge"],"Hostnames");
}
...
function IPsStoredEvents(){
    StoredEvents.call(this,"fas fa-desktop","ip","/cgi-bin/rtcGetIPs.py",[" Penalty","ANC"," CTR","AMP","UMB","SW","Flows","DLP Report","Details","IP","Purge"],"IP Addresses");
}
...
function getStarted() {
    ...
    gl_sma_config = new SMAconfig();
    gl_duo_config = new DUOconfig();
...
function dynamicOptionsResponse(xhr) {
	...
	if ( (item == "adconfig") || (item == "all")) {
	    ...
	    gl_ad_config.setADgroups(adgroups);
	}
	...
	if ( (item == "swconfig") || (item == "all")) {
		...
		gl_sw_config.setHostgroups(gl_sw_hostgroup_names);
	    }
	}
...
function addCellTextarea(divRow,type,id,name,value) {
...
function addCellInput(divRow,type,id,name,value) {
    ...
    divRow.appendChild(divInput);
    if (type == "password") {
	divInput.setAttribute("class","mySecret");
	divSecret = document.createElement("div");
	divSecret.setAttribute("class","hide");
	divSecret.innerHTML = value;
	divRow.appendChild(divSecret);
...
function mEditSMA() {
    gl_view = "";        
    gl_sma_config.getConfig();
}
function mEditDUO() {
    gl_view = "";        
    gl_duo_config.getConfig();
}
...
function mDLPreport(username="",email="") {
    //todo share code with searchapi
    gl_view = "";    
    var divChild = startEdit();
    addRow(divChild,"DLP Report","","divHeaderRow");
    
    var divRow = addCellRow(divChild);
    addCellStatic(divRow,"Search Constraints");    
    divRow = addCellRow(divChild);
    addCellStatic(divRow,"Email");
    addCellInput(divRow,"text","email","email",email);
    divRow = addCellRow(divChild);
    addCellStatic(divRow,"Username");
    addCellInput(divRow,"text","username","username",username);
    
    divRow = addCellRow(divChild);
    addCellStatic(divRow,"DAYS");
    addCellNumber(divRow,"days","days",1,0,100);

    divRow = addCellRow(divChild);
    addCellStatic(divRow,"HOURS");
    addCellNumber(divRow,"hours","hours",0,0,23);

    divRow = addCellRow(divChild);
    addCellStatic(divRow,"MINUTES");
    addCellNumber(divRow,"minutes","minutes",0,0,59);	    

    divRow = addCellRow(divChild);
    addCellButton(divRow,"Submit","Search API","submit",cbDLP);
    addCellButton(divRow,"Cancel","Cancel","submit",mAbout);
...
function DLPreportResponse(dlp_xhr) {

    stopTimeGlass();
    var rsp = JSON.parse(dlp_xhr.responseText);
    if (rsp.rtcResult != "OK") {
	alert("Error :" + rsp.rtcResult);
	return
    }
    ...
    var smadata = []
    smadata = rsp.smadata;
    duodata = rsp.duo.response.authlogs;

    var flowWindow = window.open("rtcreport.html","");
    flowWindow.onload = function() {

	flowWindow.document.title = "DLP Report";
	var divDetails = buildDivNewWindow(flowWindow,"DLP Report","");    
	addRow(divDetails,"Email DLP Reporting","","divHeaderRow");
	var divHeadRow = tableHeader(divDetails);

	addHeaderCell(divHeadRow,"Sender");
	addHeaderCell(divHeadRow,"Receiver");
	addHeaderCell(divHeadRow,"DLP Policy");
	addHeaderCell(divHeadRow,"DLP Severity");
	addHeaderCell(divHeadRow,"DLP Match");
	addHeaderCell(divHeadRow,"DLP Delivery Status");
	addHeaderCell(divHeadRow,"DLP Risk Factor");
	addHeaderCell(divHeadRow,"Attachments");
	addHeaderCell(divHeadRow,"Message Size");

	for (var i=0;i<smadata.length;i++) {

	    d = smadata[i]["messages"];

            var divRow = document.createElement("div");
	    divDetails.appendChild(divRow);
            divRow.setAttribute("class","divRow3");
            addCell(divRow,d["sender"]);	
            addCell(divRow,d["recipient"]);	
            addCell(divRow,d["dlpDetails"]["dlpPolicy"]);	
            addCell(divRow,d["dlpDetails"]["violationSeverity"]);	
            addCell(divRow,JSON.stringify(d["dlpDetails"]["dlpMatchedContent"]));	
            addCell(divRow,d["messageStatus"]);	
            addCell(divRow,d["dlpDetails"]["riskFactor"]);	
            addCell(divRow,d["attachments"]);	
            addCell(divRow,d["messageSize"]);	
	...
	addHeaderCell(divHeadRow,"Time");
	addHeaderCell(divHeadRow,"OS");
	addHeaderCell(divHeadRow,"Application");
	addHeaderCell(divHeadRow,"Browser");
	addHeaderCell(divHeadRow,"Browser Version");
	addHeaderCell(divHeadRow,"IP");
	addHeaderCell(divHeadRow,"Location");
	...
	for (var i=0;i<duodata.length;i++) {
	    ...
	    addCell(divRow,date);	
	    addCell(divRow,d["access_device"]["os"]);	
	    addCell(divRow,d["application"]["name"]);	
	    addCell(divRow,d["access_device"]["browser"]);	
	    addCell(divRow,d["access_device"]["browser_version"]);	
	    addCell(divRow,d["access_device"]["ip"]);	
	    addCell(divRow,d["access_device"]["location"]["city"]);	
        }

    }
    updateRetCode("OK","DLP Report")
